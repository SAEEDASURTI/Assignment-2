...
def test_neuron():
    ...
    assert calculate_matrix_prod_with_bias(X, W, b).tolist() == [[1.5]], "wrong output"
    assert isinstance(calculate_matrix_prod_with_bias(X, W, b), torch.Tensor), "not a tensor"

    assert calculate_activation(torch.tensor(0.0)).tolist() == 0.0, "wrong activation"
    assert calculate_activation(torch.tensor(0.1)).tolist() == 1.0, "wrong activation"
    assert calculate_activation(torch.tensor(-0.1)).tolist() == 0.0, "wrong activation"
    assert isinstance(calculate_activation(torch.tensor(0.0)), torch.Tensor), "not a tensor"

    assert calculate_output(X, W, b).tolist() == [[1.0]], "wrong output"
